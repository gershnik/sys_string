...
Whichever method you use, you can set the following macros (either on the command line or _before_ including any library headers) to control the library behavior:

* `SYS_STRING_NO_S_MACRO` - set it to 1 to disable the short `S()` macro. See [Usage](doc/Usage.md#basics) for details.
...
* `SYS_STRING_USE_ICU` - set it to 1 to use ICU instead of internal code for case conversion, normalization and grapheme iteration. ICU headers
  must be present on the include path and available via `#include <unicode/...>`. You will also need to link with ICU libraries.
